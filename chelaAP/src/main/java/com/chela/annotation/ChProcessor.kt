...
class ChProcessor : AbstractProcessor(){
    ...
    val styles = (
        ...
        "image," +
        "client,chromeClient,setting,optimization,debuggingEnabled,javascriptEnabled,localstorageEnabled,openWindowEnabled,cacheEnabled,cacheMode,fitContent,supportZoom,encoding,allowFileAccess,loadsImagesAutomatically,usePlugin,mediaPlaybackRequiresUserGesture,userAgent,loadUrl,loadJavascript,back,reload,clearCache," +
    ...
    override fun process(set: MutableSet<out TypeElement>?, roundEnvironment:RoundEnvironment?):Boolean{
        ...
        roundEnvironment?.getElementsAnnotatedWith(STYLE::class.java)?.forEach{
            ...
            fields.find{!styles.contains(it.toLowerCase())&& !exProp.contains(it.toLowerCase()) && !plugin.contains(it.toLowerCase())}?.let{
