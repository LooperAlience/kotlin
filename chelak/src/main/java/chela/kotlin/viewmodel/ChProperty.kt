...
sealed class Property{abstract operator fun invoke(view:View, v:Any)}
object ChProperty{
    ...
    @JvmStatic fun f(view:View, k:String, v:Any) = properties[k.toLowerCase()]?.invoke(view, v)
...
object Click:Property(){override fun invoke(view:View, v:Any){
    if(v !is View.OnClickListener) return
    view.isClickable = true
    view.setOnClickListener(v)
}}
object Display:Property(){override fun invoke(view:View, v:Any){
    if(v is Boolean) view.visibility = if(v) View.VISIBLE else View.GONE
}}
object Visibility:Property(){override fun invoke(view:View, v:Any){
    if(v is Boolean) view.visibility = if(v) View.VISIBLE else View.GONE
    else if(v is Number) view.visibility = v.toInt()
}}
object Background:Property(){override fun invoke(view:View, v:Any){
    if(v !is String) return
    view.setBackgroundColor(color(v))
}}
object Shadow:Property(){override fun invoke(view:View, v:Any){
    if(v !is Number || Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) return
    view.elevation = v.toFloat()
}}
object X:Property(){override fun invoke(view:View, v:Any){
    if(v is Number) view.translationX = v.toFloat()
}}
object Y:Property(){override fun invoke(view:View, v:Any){
    if(v !is Number) return
    view.translationY = v.toFloat()
}}
object Z:Property(){override fun invoke(view:View, v:Any){
    if(v !is Number || Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) return
    view.translationZ = v.toFloat()
}}

object Text: Property(){override fun invoke(view: View, v:Any){
    if(view !is TextView) return
    view.text = v as String
}}
object TextSize: Property(){override fun invoke(view: View, v:Any){
    if(v !is Number || view !is TextView) return
    view.textSize = v.toFloat()
}}
object TextColor: Property(){override fun invoke(view: View, v:Any){
